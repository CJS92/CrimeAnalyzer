    ...
    internal class Crimes
    {
        ...
        public Crimes(string[] args)
        {
            ...
            else
            {
                if (args.Length != 2)
                {
                    Console.WriteLine("Error - Try again using this format: CrimeAnalyzer <csv_file_path> <report_file_path>");
                    Console.ReadLine();
            ...
            if (File.Exists(csvFile))
            {
                if (ReadData(csvFile))
                {
                    ...
                    {
                        Console.WriteLine($"Unable to create the report file.");
                    }
            ...
            {
                Console.Write($"Error: Unable to access data file.");
            }
        ...
        private static bool ReadData(string filePath)
        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                crimeData.ViolentCrime = Convert.ToInt32(data[2]);
                            ...
                            {
                                Console.WriteLine($"Row {index} contains invalid information.");
                                return false;
                            }
                        }
                    }
            ...
            {
                Console.WriteLine("Error: Unable to read from the CSV file.");
                throw ex;
            }
        }
        ...
        private static void WriteReport(string filePath)
        {
            try
            {
                if (CrimeDataList != null && CrimeDataList.Any())
                {
                    ...
                    newString.Append($"Time Frame: {minYear}-{maxYear} ({years} years)");
                    newString.Append(Environment.NewLine);
                    newString.Append(Environment.NewLine);
                    ...
                    double violentCrimePerCapita = (double)violentCrimeData.ViolentCrime / (double)violentCrimeData.Population;
                    ...
                    newString.Append($"Average murder per year (total): {avgMurders}");
                    newString.Append(Environment.NewLine);
                    ...
                    newString.Append($"Year with the highest number of motor vehicle thefts: {maxVehicleTheft}");
                    newString.Append(Environment.NewLine);
            ...
            {
                Console.WriteLine("Error writing to or accessing the report file...");
                throw ex;
            }
        }
    }
